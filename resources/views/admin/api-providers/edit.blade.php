...
@section('page_title', 'Edit API Provider - ' . $provider->api_name)

@section('content')
<div class="max-w-2xl">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <form action="{{ route('admin.api-providers.update', $provider) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="api_name" id="api_name" value="{{ old('api_name', $provider->api_name) }}"
            ...
            <div>
                ...
                <input type="text" name="short_name" id="short_name" value="{{ old('short_name', $provider->short_name) }}"
            ...
            <div>
                ...
                <input type="url" name="api_url" id="api_url" value="{{ old('api_url', $provider->api_url) }}"
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Leave blank to keep current key</p>
                @error('api_key')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white @error('status') border-red-500 @enderror">
                    <option value="enabled" @selected(old('status', $provider->status) === 'enabled')>Enabled</option>
                    <option value="disabled" @selected(old('status', $provider->status) === 'disabled')>Disabled</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 space-y-2">
                ...
                <div class="flex justify-between">
                    <span class="text-gray-600 dark:text-gray-300">Services Count:</span>
                    <span class="font-semibold">{{ $provider->db_services_count ?? $provider->services()->count() }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600 dark:text-gray-300">Last Synced:</span>
                    <span class="font-semibold">{{ $provider->last_sync_at ? $provider->last_sync_at->diffForHumans() : 'Never' }}</span>
                </div>
            </div>
