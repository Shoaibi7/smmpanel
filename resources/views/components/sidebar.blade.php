<aside    <aside
    ...
    >
        ...
        <div class="h-20 flex items-center px-6 relative z-10 shrink-0">
            <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('dashboard') }}" class="flex items-center gap-3 group w-full" wire:navigate>
                <div class="relative">
        ...
        <div class="flex-1 overflow-y-auto overflow-x-hidden py-4 px-4 space-y-8 scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] relative z-10">
            ...
            <!-- Main Section -->
            <div class="space-y-1">
                <x-sidebar-link :href="auth()->user()->role === 'admin' ? route('admin.dashboard') : route('dashboard')" :active="request()->routeIs('dashboard') || request()->routeIs('admin.dashboard')" icon="home" wire:navigate>
                    Dashboard
                </x-sidebar-link>
        ...
        <div class="p-4 relative z-10 shrink-0">
            ...
            <div class="p-3 bg-secondary-50/50 dark:bg-black/20 backdrop-blur-md rounded-2xl border border-secondary-200/50 dark:border-secondary-700/50 flex items-center gap-3">
                <div class="relative shrink-0">
                    <img class="w-9 h-9 rounded-lg object-cover border border-secondary-200 dark:border-secondary-700" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=f97316&color=fff" alt="User">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold text-secondary-900 dark:text-white truncate">{{ auth()->user()->name }}</p>
